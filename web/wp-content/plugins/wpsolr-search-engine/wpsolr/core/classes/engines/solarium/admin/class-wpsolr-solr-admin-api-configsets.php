...
class WPSOLR_Solr_Admin_Api_ConfigSets extends WPSOLR_Solr_Admin_Api_Abstract {
	...
	static function get_config_file_path() {
		...
		return self::zip_config_files( plugin_dir_path( __FILE__ ) . 'conf/5.0/',
			'wpsolr-v5.zip',
			[
				'elevate.xml',
				'schema.xml',
	...
	static function zip_config_files( $config_dir, $filename, $files ) {

		/**
		 * Create the zip if not here
		 */
		$zip = new ZipArchive();


		if ( $zip->open( $config_dir . $filename, ZipArchive::CREATE ) !== true ) {
			exit( "Could not open file <$filename>\n" );
		}

		foreach ( $files as $file ) {
			$zip->addFile( $config_dir . $file, $file );
		}
		$zip->close();

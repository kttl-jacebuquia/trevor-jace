...
function return_solr_instance() {
	...
	if ( isset( $_POST['security'] ) && wp_verify_nonce( $_POST['security'], WPSOLR_NONCE_FOR_DASHBOARD ) ) {
		...
		$index_engine = isset( $_POST['sindex_engine'] ) ? sanitize_text_field( $_POST['sindex_engine'] ) : WPSOLR_AbstractEngineClient::ENGINE_SOLR;

		$slabel   = sanitize_text_field( $_POST['slabel'] );
		$spath    = sanitize_text_field( $_POST['spath'] );
		$port     = sanitize_text_field( $_POST['sport'] );
		$host     = sanitize_text_field( $_POST['shost'] );
		$username = sanitize_text_field( $_POST['skey'] );
		$password = sanitize_text_field( $_POST['spwd'] );
		$protocol = sanitize_text_field( $_POST['sproto'] );

		$solr_cloud_extra_parameters = [];
		switch ( $index_engine ) {
			case WPSOLR_AbstractEngineClient::ENGINE_SOLR_CLOUD:
				$solr_cloud_extra_parameters['index_solr_cloud_shards']             = isset( $_POST['index_solr_cloud_shards'] ) ? sanitize_text_field( $_POST['index_solr_cloud_shards'] ) : '2';
				$solr_cloud_extra_parameters['index_solr_cloud_replication_factor'] = isset( $_POST['index_solr_cloud_replication_factor'] ) ? sanitize_text_field( $_POST['index_solr_cloud_replication_factor'] ) : '2';
				$solr_cloud_extra_parameters['index_solr_cloud_max_shards_node']    = isset( $_POST['index_solr_cloud_max_shards_node'] ) ? sanitize_text_field( $_POST['index_solr_cloud_max_shards_node'] ) : '2';
				break;

			case WPSOLR_AbstractEngineClient::ENGINE_ELASTICSEARCH:
				$solr_cloud_extra_parameters['index_elasticsearch_shards']   = isset( $_POST['index_elasticsearch_shards'] ) ? sanitize_text_field( $_POST['index_elasticsearch_shards'] ) : '5';
				$solr_cloud_extra_parameters['index_elasticsearch_replicas'] = isset( $_POST['index_elasticsearch_replicas'] ) ? sanitize_text_field( $_POST['index_elasticsearch_replicas'] ) : '1';
				break;
		}
		...
		try {
			...
			$action = isset( $_POST['wpsolr_index_action'] ) ? sanitize_text_field( $_POST['wpsolr_index_action'] ) : 'wpsolr_index_action_ping';
...
function return_solr_index_data() {
	...
	if ( isset( $_POST['security'] ) && wp_verify_nonce( $_POST['security'], WPSOLR_NONCE_FOR_DASHBOARD ) ) {
		try {
			...
			$solr_index_indice = sanitize_text_field( $_POST['solr_index_indice'] );
...
function return_solr_delete_index() {
	...
	if ( isset( $_POST['security'] ) && wp_verify_nonce( $_POST['security'], WPSOLR_NONCE_FOR_DASHBOARD ) ) {
		try {
			...
			$post_types = isset( $_POST['post_types'] ) ? sanitize_text_field( $_POST['post_types'] ) : null;
